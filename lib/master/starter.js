...
starter.run = function(app, server, cb) {
  ...
  if (utils.isLocal(server.host)) {
    ...
    if (env === Constants.RESERVED.ENV_PRO) {
      // Use forever to start servers on production.
      var mainFile = app.get(Constants.RESERVED.MAIN);
      var logFile = path.join(path.dirname(mainFile), 'logs', 'forever.log');
      var childLogFile = path.join(path.dirname(mainFile), 'logs', 'forever-' + server.id + '.log');
      forever.startDaemon(cmd, { args: options, logFile: logFile, outFile: childLogFile, errFile: childLogFile } )
    } else {
      options.unshift(cmd);
      starter.localrun(process.execPath, null, options, cb);
    }
  } else {
    cmd = util.format('cd "%s" && ', app.getBase());
    var arg = server.args;
    if (arg !== undefined) {
      cmd += arg;
    }

    if (env === Constants.RESERVED.ENV_PRO || env === 'beta') {
      var logFile = path.join(path.dirname(mainFile), 'logs', 'forever.log');
      var childLogFile = path.join(path.dirname(mainFile), 'logs', 'forever-' + server.id + '.log');
      cmd += util.format('forever -al %s -ao %s -ae %s start "%s" env=%s ', logFile, childLogFile, childLogFile, app.get(Constants.RESERVED.MAIN), env);
    } else {
      cmd += util.format('"%s" "%s" env=%s ', process.execPath, app.get(Constants.RESERVED.MAIN), env);
    }
    for(key in server) {
      if(key === Constants.RESERVED.CPU) {
        cpus[server.id] = server[key];
