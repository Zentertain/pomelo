...
Application.loadConfigBaseApp = function (key, val, reload) {
  this.loadConfig(key, val, reload);
  return;
};
 ...
 * @memberOf Application
 */
Application.loadConfig = function(key, val, reload) {
  var env = this.get(Constants.RESERVED.ENV);
  ...
  if(fs.existsSync(presentPath)) {
    ...
    var envConfig = require(presentPath);
    if (key === Constants.RESERVED.SERVERS || key === Constants.RESERVED.MASTER) {
      config = envConfig;
    } else {
      config = __.merge(config, envConfig);
    }
  }

  if (!config) {
    logger.error('invalid configuration with file path: %s', val);
  }

  this.set(key, config);

  // TODO: How should we handle change watch if envConfig overwrite basic config partly?
  if(!!realPath && !!reload) {
    var self = this;
    fs.watch(realPath, function (event, filename) {
      if(event === 'change') {
        delete require.cache[require.resolve(realPath)]
        self.loadConfig(key, val);
      }
    });
  }
};
